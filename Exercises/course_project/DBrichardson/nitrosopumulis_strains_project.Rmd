...
The main focus of this work is the exploration of strain-level diversity in Nitrosopumulis sp. in Narragansett Bay. To achieve this goal, we:
  ...
  4. Selected a representative Nitrosopumulis MAG from the metagenomic time series
  5. Identify variants 
  6. Cluster samples by conspecfic varaint composition 

Part 1: NGS read quality filtering 

This slurm array job begins with combining the reads from the separate lanes into forward and reverse read files for each sample. Next, PCR duplicates are removed using the clumpify.sh module in the BBtools (v.38.87) software package. The overlapping deduplicated reads were then merged together using bbmerge.sh from the BBtools package. Next the adapters are removed from the reads, low-quality bases are trimmed from the reads, and the quality-trimmed reads that are shorter 251 bp long are dropped using Trimmomatic (v. 0.39). The unmerged reads can suffer from an abundance of erroneous homopolymer repeats. To remove these reads, AfterQC (v. 0.9.7) was used. In preparation for metagenomic read assembly, the quality filtered reads were normalized  to 100 - i.e. reads containing k-mers observed more than 100 times in the sample were removed - using the bbnorm.sh script from BBtools. 
...
To study Nitrosopumilis population structure, the  short NGS reads were assembled into contigs using metaSPAdes (v. 3.15.2). This program can be very memory intensive, so it was run using the High Performance Computing Clusters Andromeda and Bluewaves. When Bluewaves was used, it was necessary to move the files from the data directory to the data4 directory. 
```{bash}
#!/bin/bash
#SBATCH -t 100:00:00
#SBATCH --nodes=1
...
cd /data/zhanglab/dbrichardson/projects/NB/analyses/02_Assembly/tests/
...
This array job builds a bowtie2 index for each contig in every  metagenome assembly using Bowtie2 (v. 2.4.1). It then calls the script below to map the quality filtered reads to the contigs. 
```{bash}
#!/bin/bash
#SBATCH -t 100:00:00
#SBATCH --array=1-68%2
...
#This spaces out the array, so that it doesn't monopolize bluewaves completely.
sleep 10h
...
This array job uses Bowtie2 to align the quality filtered reads from each sample to all of the contigs produced by metaSPAdes. Then, it uses samtools (v. 1.10) to convert the sam files to bam files, to sort the bam files, and then index the bam files. 
...
#Copying read mappings to binning directory
...
#Removing read mappings from binning directory
rm ${BAM_FILES_SUBSET}/*
...
#Load the MetaBat2 modle to make the contig depth files
module load MetaBAT/2.12.1-foss-2018b-Python-2.7.15
...
#Using metaWRAP pipline guidence
...
```{bash}
cat ${clust_by_taxonomy_DIR}/sp_level_rep_MAGS.list | grep "Nitrosopumulis" 
```
...
This filters the variants
```{r}
library(vcfR)
library(ape)

#reading in the VCF
my_vcf <- read.vcfR("~/research_data/NB_Nitrosopumilus_all_jc.g.vcf.gz")
#Reading in the representative MAG
dna <- ape::read.dna("~/research_data/ZTP33_S7_bin_42_completeness_76_27_contamination_0_970_length_900089.fa", format="fasta")
#Reading in the called genes 
gff <- read.table("~/research_data/nitrosopumilus_genes.gff", sep="\t", quote="")

chrom <- create.chromR(name="Nitro Genome", vcf=my_vcf, seq=dna, ann=gff)
...
```
Uploading filtered VCF into R and inspecting it
```{r}
#reading in the filtered VCF file.
my_vcf2 <- read.vcfR("~/research_data/NB_Nitrosopumilus_all_jc.snps_hq_filtered.vcf.gz")

#Creating a chromR object that concatenates all of the contigs into a single chromosome 
chrom2 <- create.chromR(name="Nitro Genome", vcf=my_vcf2, seq=dna, ann=gff)
...
s.label(pca1$li, clabel=0.3) + title("PCA of Nitrosopumilus dataset\naxes 1-2") + add.scatter.eig(pca1$eig[1:20], 3,1,2)
colorplot(pca1$li, pca1$li, transp=TRUE, cex=0.5, xlab="PC 1", ylab="PC 2" )

grp <- find.clusters(my_genind2, max.n.clust=48)#PCs 20, clusters 5
names(grp)

head(grp$grp, 10)
grp$size
dapc1 <- dapc(my_genind2, grp$grp)# PCs to retain 3, disc functions 2
scatter(dapc1)
