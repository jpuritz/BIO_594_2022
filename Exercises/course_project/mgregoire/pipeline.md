...
Pumpkin got his DNA sequenced through Basepaws! He got a mouth swab and sent it in for sequencing and analysis. Basepaws delivered a report of Pumpkin's SNPs as well as his teeth microbiome and a food analysis. Below is a pipeline I have established for some analyses that can be performed on the resulting fastq and vcf files provided by Basepaws.
...
The vcf file can be looked at by using `cat EFF.R163.vcf`. When this is typed, the vcf file is printed to the terminal screen and the constant printing can be interrputed with `ctrl c`. The columns in this vcf file show that the annotations do not seem to be right, as the chromosome numbers are not correct. Cats have 38 chromosomes A1-A3, B1-B4, C1-C2, D1-D4, E1-E3, F1-F2, X, Y. None of these chromosomes were properly indentified. This error could be due to not knowing the nature of the fasta files. R163 was the only file that was small enough to be aligned to the cat reference genome. It could be Pumpkin's DNA, or it could be DNA used for his teeth microbiome analysis, or food analysis. Because of this, the following steps will use the vcf file provided by Basepaws that has proper alignment of Pumpkin's own cat DNA to the cat reference genome and proper annotations. 
...
![qcchrom](https://github.com/jpuritz/BIO_594_2022/tree/main/Exercises/course_project/mgregoire/qcrhom.png)
...
I never knew that the analysis behind all of Pumpkin's DNA would take so much work. I spent a couple weeks analyzing his DNA for this project and did not get anywhere near the extent of analysis that Basepaws did. It is truly amazing what can be done when you know what to look for and have the right equipment and an established pipeline. Kudos to all the wonderful people at Basepaws who made both analyzing Pumpkin's DNA for this project possible, and also knowing all about his health :)
